...
V4l2MmapDevice::V4l2MmapDevice(const V4L2DeviceParameters& params, v4l2_buf_type deviceType)
    : V4l2Device(params, deviceType), n_buffers(0)
{
...
{
    //    cout << "[NOTICE]"
    //         << "Device " << m_params.m_devName << endl;
    ...
    if (-1 == ioctl(m_fd, VIDIOC_REQBUFS, &req))
    {
        ...
        {
            cout << "[ERROR]"
                 << "Device " << m_params.m_devName << " does not support memory mapping" << endl;
            success = false;
        }
        else
        {
    ...
    {
        //        cout << "[NOTICE]"
        //             << "Device " << m_params.m_devName << " nb buffer:" << req.count << endl;
        ...
        for (n_buffers = 0; n_buffers < req.count; ++n_buffers)
        {
            ...
            {
                //                cout << "[INFO]"
                //                     << "Device " << m_params.m_devName << " buffer idx:" << n_buffers << " size:" << buf.length << " offset:" << buf.m.offset << endl;
    ...
    if (n_buffers > 0)
    {
        ...
        else if (buf.index < n_buffers)
        {
            ...
            if (size > bufferSize)
            {
                size = bufferSize;
                cout << "[WARN]"
                     << "Device " << m_params.m_devName << " buffer truncated available:" << bufferSize << " needed:" << buf.bytesused << endl;
            }
...
size_t V4l2MmapDevice::writeInternal(char* buffer, size_t bufferSize)
{
    ...
    if (n_buffers > 0)
    {
        ...
        else if (buf.index < n_buffers)
        {
            ...
            {
                cout << "[WARN]"
                     << "Device " << m_params.m_devName << " buffer truncated available:" << buf.length << " needed:" << size << endl;
                size = buf.length;
            }
...
size_t V4l2MmapDevice::writePartialInternal(char* buffer, size_t bufferSize)
{
    ...
    if ((n_buffers > 0) && m_partialWriteInProgress)
    {
        if (m_partialWriteBuf.index < n_buffers)
        {
            ...
            {
                cout << "[WARN]"
                     << "Device " << m_params.m_devName << " buffer truncated available:" << m_partialWriteBuf.length << " needed:" << new_size << endl;
