...
void Rfb::handlePointerEvent(const QPointF& pos, quint8 buttonMask, QWindow* window)
{
    ...
    else
    {
        ...
        {
            eventType = (buttons > oldButtons) ? QEvent::MouseButtonPress : QEvent::MouseButtonRelease;
        ...
        Q_UNUSED(eventType)
        Q_UNUSED(button)
        QWindowSystemInterface::handleMouseEvent(window, pos, pos, buttons, keyboardModifiers);  //eventType
    }
}
